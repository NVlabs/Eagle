...
class Siglip2Attention(nn.Module):
    ...
    def forward(
        ...
        # if windows_attn:
        #     seq_length = [win_meta['win_hw'][0] * win_meta['win_hw'][1] for win_meta in win_meta_list]
        # else:
        #     mapper = defaultdict(lambda: 0)
        #     for win_meta in win_meta_list:
        #         mapper[win_meta['img_idx']]  += win_meta['win_hw'][0] * win_meta['win_hw'][1]
        #     seq_length = [mapper[i] for i in range(len(mapper))]
        # attention_mask = torch.cat([
        #     torch.full((shape,), i, dtype=torch.long)
        #     for i, shape in enumerate(seq_length)
        # ])
